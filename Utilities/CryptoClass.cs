...
namespace Utilities
{
    public sealed class CryptoClass
    {
        ...
        //private static string encryptionKey = "667B9BE6F5BB59FBCBAC5ACFA30AE05B";
        //-------UAT
        //private static string encryptionKey = "45F5F6CE810E03611A48F7B5C612B375";
        //-------Production
        private static string encryptionKey = "F6E115612C14591072E4EB6D41A89D14";
        ...
        public string DecryptText(string encryptedString)
        {
            using (myRijndael = new RijndaelManaged())
            {
                ...
                //myRijndael.GenerateIV();  // myRijndael.IV = HexStringToByte(initialisationVector);
                myRijndael.Mode = CipherMode.CBC;
                myRijndael.Padding = PaddingMode.PKCS7;

                byte[] cipherBytes = Convert.FromBase64String(encryptedString);
                byte[] IV = new byte[myRijndael.BlockSize / 8];
                byte[] cipherT = new byte[cipherBytes.Length - IV.Length];
                Array.Copy(cipherBytes, IV, IV.Length);
                Array.Copy(cipherBytes, IV.Length, cipherT, 0, cipherT.Length);

                myRijndael.IV = IV;

                encryptedString = Convert.ToBase64String(cipherT);
        ...
        public string EncryptText(string plainText)
        {
            using (myRijndael = new RijndaelManaged())
            {
                ...
                string encString = Convert.ToBase64String(combinedIvCt);
